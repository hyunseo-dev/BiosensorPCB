...
void initialCalibration_old() {
  ...
  if (AD5933::calibrate(gain, phase, refResist, numIncrements + 1)) {
...
void showSweepMenu() {
  ...
  while (true) {
    if (Serial.available() > 0) {
      ...
      if (startFreq >= 1 && startFreq <= 100) {
        ...
        Serial.print("1~100 kHz의 값을 입력하세요: ");
      }
    }
  }
  ...
  while (true) {
    if (Serial.available() > 0) {
      ...
      if (frequencyUnit >= 1 && frequencyUnit <= 1000) {
        ...
        Serial.println("\t 잘못된 입력입니다.");
        Serial.print("1~1000 Hz의 값을 입력하세요: ");
      }
    }
  }
  ...
  while (true) {
    if (Serial.available() > 0) {
      ...
      Serial.print(settlingTimeCycles);
      if (settlingTimeCycles >= 0 && settlingTimeCycles <= 511) {
        ...
        Serial.println("\t 잘못된 입력입니다.");
        Serial.print("0~511 사이의 값을 입력하세요: ");
      }
    }
  }
  ...
  while (true) {
    if (Serial.available() > 0) {
      ...
      Serial.println(excitationRange);
      switch (excitationRange) {
        case 1:
          ad5933.setRange(CTRL_OUTPUT_RANGE_1);
          Serial.println("2 Vpp (Range 1)");
          break;
        case 2:
          ad5933.setRange(CTRL_OUTPUT_RANGE_2);
          Serial.println("1 Vpp (Range 2)");
          break;
        case 3:
          ad5933.setRange(CTRL_OUTPUT_RANGE_3);
          Serial.println("0.4 Vpp (Range 3)");
          break;
        case 4:
          ad5933.setRange(CTRL_OUTPUT_RANGE_4);
          Serial.println("0.2 Vpp (Range 4)");
          break;
        default:
          Serial.println("잘못된 입력입니다.");
          Serial.print("1~4 중 하나를 선택하세요: ");
          continue;
      }
      break;
    }
  ...
  while (true) {
    if (Serial.available() > 0) {
      ...
      if (pgaControl == 1 || pgaControl == 5) {
        ...
        Serial.println("잘못된 입력입니다.");
        Serial.println("1 또는 5를 입력하세요: ");
      }
    }
  }

  // Calibration Impedance 입력
  Serial.print("Calibration Impedance를 입력하세요 (옴 단위): ");
  while (true) {
    if (Serial.available() > 0) {
      ...
      if (refResist > 0) {
        ...
        Serial.println("\t 잘못된 입력입니다.");
        Serial.print("올바른 Calibration Impedance 값을 입력하세요: ");
      }
    }
  }
...
void getAddressInput() {
    ...
    // X축 주소 설정 및 확인
    Serial.println("X축 Address (7자리, 각 비트는 0 또는 1로 입력):");
    for (int i = 0; i < 7; i++) {
        while (1) {
            if (Serial.available() > 0) {
                String inputStr = Serial.readStringUntil('\n');
                inputStr.trim(); // 공백 제거
                int bitValue = inputStr.toInt();
