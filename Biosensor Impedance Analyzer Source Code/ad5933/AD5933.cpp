 ...
 */
// !! Control Register 주소 0x80,  0x81. 값을 잘못 보냈을 경우 0xA000 할당으로 초기화(디폴트값) 0x80(CTRL_REG1) = 'A0' 0x81(CTRL_REG2) = '00' !!
...
bool AD5933::getComplexData(int *real, int *imag) {
    ...
    // 상태 레지스터 확인
    if ((readStatusRegister() & STATUS_DATA_VALID) == STATUS_DATA_VALID) {
        // 데이터 레지스터에서 실수와 허수 데이터 읽기
        byte realComp[2];
        byte imagComp[2];
        if (getByte(REAL_DATA_1, &realComp[0]) &&
            getByte(REAL_DATA_2, &realComp[1]) &&
        ...
        Serial.println("Data not valid after waiting.");
        *real = 0;
        *imag = 0;
        return false;
    }
...
bool AD5933::calibrate(double gain[], double phase[], int ref, int n) {
    ...
    // 정확성 향상을 위해 위상 double 형식으로 변경함(레지스터에 저장되는 것이 아니므로 int<->double 변경 가능)
    int *real = new int[n];
    int *imag = new int[n];
...
bool AD5933::calibrate(double gain[], double phase[], int real[], int imag[],
                       int ref, int n) {
    // Perform the frequency sweep
    // 현재 코드에서 미사용; 코드 정확성 향상을 위해 위상 double 형식으로 변경함(레지스터에 저장되는 것이 아니므로 int<->double 변경 가능)
    if (!frequencySweep(real, imag, n)) {
        return false;
    }
